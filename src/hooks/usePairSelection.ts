...
export function usePairSelection(opts: {
  ...
  const updateDerivedLimits = useCallback((p: PairView) => {
    ...
    setAllowAllQuestions((p.usage?.weeklyLimit ?? p.weeklyLimit) === 0);
  ...
  const proposeGroupSettings = useCallback(async () => {
    if (!apiClient || !pair) return;
    const limit = allowAllQuestions ? 0 : Number(weeklyLimitInput);
  ...
  }, [apiClient, pair, refreshGroupSettingsPanel, allowAllQuestions, weeklyLimitInput]);

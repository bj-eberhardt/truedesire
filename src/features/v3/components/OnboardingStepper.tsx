  ...
  return (
    <ol className="v3-stepper" aria-label="Onboarding Fortschritt">
      {props.steps.map((step, idx) => {
        ...
        return (
          <li key={step.id} className="v3-step" data-state={state} aria-current={state === 'active' ? 'step' : undefined}>
            <div className="v3-step-marker" aria-hidden="true">
              <div className="v3-step-dot">
                {state === 'done' ? <span className="v3-step-check">✓</span> : <span className="v3-step-num">{idx + 1}</span>}
              </div>

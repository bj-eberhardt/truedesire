  ...
  const bodyText =
    typeof init?.body === "string"
      ...
      : init?.body !== undefined
        ...
        : "";
  const url = new URL(apiPath(inputPath), opts.baseUrl);
  ...
  return {
    auth: {
      async register(params: {
        ...
        const res = await fetch(new URL(apiPath("/auth/register"), opts.baseUrl), {
          method: "POST",
      ...
      me: () =>
        signedFetch<{ id: string; code: string; nickname: string; ecdhPublicRawB64: string }>(
          opts,
          "/auth/me"
        ),
    ...
    pairing: {
      request: (partnerCode: string) =>
        signedFetch<{ ok: true; requestId: string }>(opts, "/pairing/request", {
          method: "POST",
          body: { partnerCode }
        }),
      respond: (requestId: string, action: "accept" | "reject" | "cancel") =>
        signedFetch<{ ok: true; pairId?: string | null }>(opts, "/pairing/respond", {
          method: "POST",
    ...
    pair: {
      create: () =>
        signedFetch<{ pairId: string }>(opts, "/pair/create", { method: "POST", body: {} }),
      join: (pairId: string) =>
        signedFetch<{ ok: true }>(opts, "/pair/join", { method: "POST", body: { pairId } }),
      confirm: (pairId: string) =>
        signedFetch<{ ok: true; status: "pending" | "active" }>(opts, "/pair/confirm", {
          method: "POST",
          body: { pairId }
        }),
      proposeWeeklyLimit: (pairId: string, limit: number) =>
        signedFetch<{
          ok: true;
          weeklyLimit: number;
          pending: { id: string; proposedBy: string; limit: number; createdAt: number };
        }>(opts, "/pair/weekly-limit/propose", {
          method: "POST",
          body: { pairId, limit }
        }),
      respondWeeklyLimit: (
        ...
        signedFetch<{ ok: true; weeklyLimit: number }>(opts, "/pair/weekly-limit/respond", {
          method: "POST",
    ...
    questions: {
      ...
      list: (pairId: string) =>
        signedFetch<QuestionView[]>(opts, `/questions/${encodeURIComponent(pairId)}`),
      delete: (questionId: string) =>
        signedFetch<{ ok: true }>(opts, "/questions/delete", {
          method: "POST",
          body: { questionId }
        })
    },

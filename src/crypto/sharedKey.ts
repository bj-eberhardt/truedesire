...
async function hkdfAesKey(secret: Uint8Array, saltText: string, infoText: string): Promise<CryptoKey> {
  const hkdfKey = await crypto.subtle.importKey('raw', bytesToArrayBuffer(secret), 'HKDF', false, ['deriveKey'])
  const salt = utf8ToBytes(saltText)
  const info = utf8ToBytes(infoText)
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: bytesToArrayBuffer(salt), info: bytesToArrayBuffer(info) },
    hkdfKey,
...
export async function derivePairAesKey(ecdhPrivateKey: CryptoKey, pair: PairView, myUserId: string): Promise<CryptoKey> {

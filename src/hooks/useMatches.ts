...
export type MatchView = { id: string; question: string; grade: 'perfect' | 'maybe' | 'ok'; answers: AnswerChoice[] }

type UseMatchesResult = {
  matches: MatchView[]
  isLoadingMatches: boolean
  clearMatches: () => void
  computeMatches: (pairOverride?: PairView, rawQuestionsOverride?: QuestionView[], decryptedQuestionsOverride?: DecryptedQuestion[]) => Promise<void>
}

export function useMatches(opts: { apiClient: ApiClient | null; identity: Identity | null; pair: PairView | null; minLoadingMs?: number }) {
  ...
  const computeMatches = useCallback(
    async (pairOverride?: PairView, rawQuestionsOverride?: QuestionView[], decryptedQuestionsOverride?: DecryptedQuestion[]) => {
      ...
      try {
        ...
        const questionSource = rawQuestionsOverride?.length ? rawQuestionsOverride : await apiClient.questions.list(currentPair.id)
        for (const q of questionSource) {
          ...
          let questionText: string = decryptedQuestionsOverride?.find((x) => x.id === q.id)?.text ?? ''
          if (!questionText) {
            try {
        ...
        setMatches(next.map((m) => ({ id: m.id, question: m.question, grade: m.grade, answers: m.answers })))
      } finally {

...
export function useQuestions(opts: {
  ...
  const { apiClient, identity, pair, onSaved, onAnswerLimitReached, refreshCurrentPair } = opts
      ...
      try {
        ...
        const items = await Promise.all(
          system.questions.map(async (q) => ({
            systemId: q.id,
            blob: await encryptJson(aes, { text: q.text, systemId: q.id, systemHash: q.sha256B64 }, `love-interests|pair:${p.id}|question|system:${q.id}`),
          })),
        )
  ...
  const loadQuestionsAndDecrypt = useCallback(
    async (pairOverride?: PairView) => {
      ...
      for (const q of list) {
        try {
          const payload = await decryptJson<{ text?: unknown; systemId?: unknown; systemHash?: unknown }>(aes, q.blob)
  ...
  const addQuestion = useCallback(
    async (text: string, selfAnswer: AnswerChoice) => {
      ...
      const blob = await encryptJson(aes, { text: trimmed }, `love-interests|pair:${pair.id}|question`)
      const created = await apiClient.questions.create(pair.id, blob)

      try {
        const answerBlob = await encryptJson(aes, { answer: selfAnswer }, `love-interests|pair:${pair.id}|answer|q:${created.id}`)
        await apiClient.answers.create(created.id, answerBlob)
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e)
        if (msg === 'weekly_limit_reached') {
          throw new Error('Frage erstellt, aber Wochenlimit fürs Antworten erreicht (deine Antwort wurde nicht gespeichert).')
        }
    ...
    async (questionId: string, choice: AnswerChoice) => {
      ...
      try {
        ...
        if (msg === 'weekly_limit_reached') {
          ...
          throw new Error('Wochenlimit fürs Antworten erreicht. Du kannst diese Woche keine weiteren Antworten abgeben.')
        }
        if (msg === 'cannot_update_after_partner_answer') {
          throw new Error('Dein Partner hat bereits geantwortet. Deine Antwort ist jetzt gesperrt und kann nicht mehr geändert werden.')
        }
        throw new Error(msg)
      }
    },
    [apiClient, identity, loadQuestionsAndDecrypt, onAnswerLimitReached, onSaved, pair, refreshCurrentPair],
  )

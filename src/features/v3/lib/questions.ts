...
export function getOpenQuestions(questions: DecryptedQuestion[], answerSummary: AnswerSummary): DecryptedQuestion[] {
...
export function countOpenNonOwnUnanswered(opts: {
  ...
  return open.filter((q) => !opts.answerSummary[q.id]?.mine && q.createdBy !== opts.identityUserId).length
}
